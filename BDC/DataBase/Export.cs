...
namespace BDC.DataBase
{
    public class Export
    {
        ...
        //    newThread.Start();
            var process = System.Diagnostics.Process.Start(runPath, path + " Result-Run-" + caseName + ".out");
        //    var process = System.Diagnostics.Process.Start(runPath, path );
            process.WaitForExit();
            string resultPath = System.AppDomain.CurrentDomain.BaseDirectory  + @"\Result-Run-" + caseName + ".out";
       //     newThread.Join();
